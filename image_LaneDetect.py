...
class LaneDetect():
    ...
    def CvImage(self,image):
        ...
        def bird(img):
            ...
            pts1 = np.float32([[width*7/25, height*7/10],[0, height],[width*8/10,height*7/10],[width, height]]) 

            # 좌표의 이동점
            pts2 = np.float32([[0,0],[0,height],[width*9/10,0],[width*8/10,height]])
        ...
        def draw_lines(img, lines, color=[155, 155, 155], thickness=5):
            avg_x = []
            avg_y = []
...
def main(args=None):
    ...
    image_link = "./photo/lane_image.png"
    

    

    img = cv2.imread(image_link, cv2.IMREAD_ANYCOLOR)
    out, left_fit, right_fit, left_detect, right_detect = lane_detect.CvImage(img)
    
    
    cv2.imshow("lane detect",out)
    cv2.waitKey(0)

    cv2.destroyWindow("lane detect")     
